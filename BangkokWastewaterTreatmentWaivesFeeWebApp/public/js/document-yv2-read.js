...
$(document).on("click", "#btn_update_document_yv2", function(event, data) {
    let formDocumentYV2 = new FormData($("#fm_document_yv2")[0]);
    updateDocumentYV2(formDocumentYV2);
});
...
function getDocumentDetailByDocNo() {
    ...
    callWebServiceTask(disableLoading, requestTypePost, urlService, dataService, dataTypeJson, cacheFalse, processDataTrue, contentTypeApplicationXWwwFormUrlencoded, function(success) {
        ...
        if (success.status == 200) {
            ...
            renderLovDistrict($("#fm_document_yv2").find("[name=district_code]"), data.lov_district);
            renderLovSubDistrict($("#fm_document_yv2").find("[name=sub_district_code]"), data.lov_sub_district);
            ...
            $("#fm_document_yv2").find("[name=doc_yv2_id]").val(data.doc_yv2_id);
            $("#fm_document_yv2").find("[name=doc_no]").val(data.doc_no);
            $("#fm_document_yv2").find("[name=address_owner]").val(data.address_owner);
            $("#fm_document_yv2").find("[name=address_name]").val(data.address_name);
            $("#fm_document_yv2").find("[name=address_code]").val(data.address_code);
            $("#fm_document_yv2").find("[name=address]").val(data.address);
            $("#fm_document_yv2").find("[name=moo]").val(data.moo);
            $("#fm_document_yv2").find("[name=soi]").val(data.soi);
            $("#fm_document_yv2").find("[name=road]").val(data.road);
            $("#fm_document_yv2").find("[name=province_code]").val(data.province_code);
            $("#fm_document_yv2").find("[name=district_code]").val(data.district_code);
            $("#fm_document_yv2").find("[name=sub_district_code]").val(data.sub_district_code);
            $("#fm_document_yv2").find("[name=zip_code]").val(data.zip_code);
            $("#fm_document_yv2").find("[name=telephone]").val(data.telephone);
            $("#fm_document_yv2").find("[name=mobile_phone]").val(data.mobile_phone);
            $("#fm_document_yv2").find("[name=fax]").val(data.fax);
            $("#fm_document_yv2").find("[name=email]").val(data.email);
            $("#fm_document_yv2").find("[name=latitude]").val(data.latitude);
            $("#fm_document_yv2").find("[name=longitude]").val(data.longitude);
            $("#fm_document_yv2").find("[name=business_type]").val(data.business_type);

            $("#fm_document_yv2").find("[name=cancel_reason]").val(data.cancel_reason);
            $("#fm_document_yv2").find("[name=cancel_date]").val(data.cancel_date);
...
function updateDocumentYV2(input) {
    let urlService = "/service/doc/updateDocumentYV2";
