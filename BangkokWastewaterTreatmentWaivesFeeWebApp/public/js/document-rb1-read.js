...
$(document).on("click", "#btn_update_document_rb1", function(event, data) {
    let formDocumentRB1 = new FormData($("#fm_document_rb1")[0]);
    updateDocumentRB1(formDocumentRB1);
});
...
function getDocumentDetailByDocNo() {
    ...
    callWebServiceTask(disableLoading, requestTypePost, urlService, dataService, dataTypeJson, cacheFalse, processDataTrue, contentTypeApplicationXWwwFormUrlencoded, function(success) {
        ...
        if (success.status == 200) {
            ...
            renderLovDistrict($("#fm_document_rb1").find("[name=district_code]"), data.lov_district);
            renderLovSubDistrict($("#fm_document_rb1").find("[name=sub_district_code]"), data.lov_sub_district);
            ...
            $("#fm_document_rb1").find("[name=doc_rb1_id]").val(data.doc_rb1_id);
            $("#fm_document_rb1").find("[name=doc_no]").val(data.doc_no);
            $("#fm_document_rb1").find("[name=address_owner]").val(data.address_owner);
            $("#fm_document_rb1").find("[name=address_name]").val(data.address_name);
            $("#fm_document_rb1").find("[name=address_code]").val(data.address_code);
            $("#fm_document_rb1").find("[name=address]").val(data.address);
            $("#fm_document_rb1").find("[name=moo]").val(data.moo);
            $("#fm_document_rb1").find("[name=soi]").val(data.soi);
            $("#fm_document_rb1").find("[name=road]").val(data.road);
            $("#fm_document_rb1").find("[name=province_code]").val(data.province_code);
            $("#fm_document_rb1").find("[name=district_code]").val(data.district_code);
            $("#fm_document_rb1").find("[name=sub_district_code]").val(data.sub_district_code);
            $("#fm_document_rb1").find("[name=zip_code]").val(data.zip_code);
            $("#fm_document_rb1").find("[name=telephone]").val(data.telephone);
            $("#fm_document_rb1").find("[name=mobile_phone]").val(data.mobile_phone);
            $("#fm_document_rb1").find("[name=fax]").val(data.fax);
            $("#fm_document_rb1").find("[name=email]").val(data.email);
            $("#fm_document_rb1").find("[name=latitude]").val(data.latitude);
            $("#fm_document_rb1").find("[name=longitude]").val(data.longitude);
            $("#fm_document_rb1").find("[name=business_type]").val(data.business_type);

            $("#fm_document_rb1").find("[name=land_owner_name]").val(data.land_owner_name);
            $("#fm_document_rb1").find("[name=land_number]").val(data.land_number);
            $("#fm_document_rb1").find("[name=land_description]").val(data.land_description);
            $("#fm_document_rb1").find("[name=land_building_type]").val(data.land_building_type);
            $("#fm_document_rb1").find("[name=connect_pipe][value='" + data.connect_pipe + "']").prop("checked", true);
            $("#fm_document_rb1").find("[name=address_connect_pipe]").val(data.address_connect_pipe);
            $("#fm_document_rb1").find("[name=pool_engineer_name]").val(data.pool_engineer_name);
            $("#fm_document_rb1").find("[name=pool_approve_date]").val(data.pool_approve_date);
            ...
            if(data.doc_attach_12 !== null){
              // $("#cb_doc_attach_12").prop("checked", true);
              $("#txt_doc_attach_12").removeClass("d-none").attr("onclick", "var openpopup = window.open('/service/doc/file/" + btoa(data.doc_attach_12) + "','filewindow12','width=1100,height=700,scrollbars=yes'); openpopup.oncontextmenu = function() { return false; } ");
            }
            if(data.doc_attach_13 !== null){
              // $("#cb_doc_attach_13").prop("checked", true);
              $("#txt_doc_attach_13").removeClass("d-none").attr("onclick", "var openpopup = window.open('/service/doc/file/" + btoa(data.doc_attach_13) + "','filewindow13','width=1100,height=700,scrollbars=yes'); openpopup.oncontextmenu = function() { return false; } ");
            }
            if(data.doc_attach_14 !== null){
              // $("#cb_doc_attach_14").prop("checked", true);
              $("#txt_doc_attach_14").removeClass("d-none").attr("onclick", "var openpopup = window.open('/service/doc/file/" + btoa(data.doc_attach_14) + "','filewindow14','width=1100,height=700,scrollbars=yes'); openpopup.oncontextmenu = function() { return false; } ");
            }
            if(data.doc_attach_15 !== null){
              // $("#cb_doc_attach_15").prop("checked", true);
              $("#txt_doc_attach_15").removeClass("d-none").attr("onclick", "var openpopup = window.open('/service/doc/file/" + btoa(data.doc_attach_15) + "','filewindow15','width=1100,height=700,scrollbars=yes'); openpopup.oncontextmenu = function() { return false; } ");
            }
            if(data.doc_attach_16 !== null){
              // $("#cb_doc_attach_16").prop("checked", true);
              $("#txt_doc_attach_16").removeClass("d-none").attr("onclick", "var openpopup = window.open('/service/doc/file/" + btoa(data.doc_attach_16) + "','filewindow16','width=1100,height=700,scrollbars=yes'); openpopup.oncontextmenu = function() { return false; } ");
            }
            if(data.doc_attach_17 !== null){
              // $("#cb_doc_attach_17").prop("checked", true);
              $("#txt_doc_attach_17").removeClass("d-none").attr("onclick", "var openpopup = window.open('/service/doc/file/" + btoa(data.doc_attach_17) + "','filewindow17','width=1100,height=700,scrollbars=yes'); openpopup.oncontextmenu = function() { return false; } ");
            }
        }
    }, function(fail) {
...
function updateDocumentRB1(input) {
    let urlService = "/service/doc/updateDocumentRB1";
