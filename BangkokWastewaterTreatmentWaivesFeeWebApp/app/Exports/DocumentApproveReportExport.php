...
class DocumentApproveReportExport implements FromCollection, ShouldAutoSize, WithEvents, WithColumnFormatting, WithStrictNullComparison
{
  ...
  */
  public function collection()
  {
    $documentLogList = DocumentLog::join("lov_document_type","lov_document_type.doc_type_code", "document_logs.doc_type")
                                  ...
                                  ->join("users","users.user_id", "document_logs.approve_by")
      ...
      $reportName = "รายชื่อผู้ได้รับการยกเว้นค่าธรรมเนียมบำบัดน้ำเสีย";
      ...
      return new Collection([
          ...
          [$reportSearch],
          [""],
          ["เลขที่คำร้อง", "รหัสผู้ใช้น้ำ", "ชื่ออาคาร/สถานประกอบการ", "ชื่อเจ้าของแหล่งกำเนิดน้ำเสีย", "ประเภทอาคาร", "วันที่ยื่นคำร้อง", "สถานะคำร้อง", "โทรศัพท์", "ผู้ตรวจสอบ"],
          $this->reportTable,
      ]);
  }
  ...
  public function registerEvents(): array
  {
      return [
          AfterSheet::class => function(AfterSheet $event) {
            ...
            $event->sheet->getStyle('A1:I1')->getFont()->setSize(16);
            $event->sheet->getStyle('A1:A2')->getFont()->setBold(true);

            $event->sheet->getStyle('A2:I2')->getFont()->setSize(13);

            $event->sheet->getStyle('A4:I4')->getFont()->setSize(11);
            $event->sheet->getStyle('A4:I4')->getFont()->setBold(true);
            $event->sheet->getStyle('A4:I4')->getAlignment()->setVertical("center");
            $event->sheet->getStyle('A4:I4')->getAlignment()->setHorizontal("center");
            $event->sheet->getStyle('A4:I4')->applyFromArray($BORDER_THIN);

            $cell = 'A5:I'.(count($this->reportTable) + 4);
